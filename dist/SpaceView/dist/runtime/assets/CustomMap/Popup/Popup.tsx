...
import { HeaderCalendarIcon, CheckIcon } from "../../../icons"
...
interface FieldConfig<T = any> {
    label: string;
    format?: (value: T, lang?: string) => string;
}
...
const fieldMap: { [key: string]: FieldConfig } = {
    ...
    "Aniqligi": {
        label: "Fazoviy aniqligi (m/pixel)"
    },
    "Spektral": {
    ...
    "Sana": {
        ...
        format: (value: any, lang: string) => { return value ? new Date(value).toLocaleDateString(lang === 'EN' ? 'en-GB' : lang === 'RU' ? 'ru-RU' : 'uz-UZ') : '' }
    },
    "Bulut_qop": {
        label: "Bulut qoplami (%)"
    },
            ...
            {props.actiom.type === 'info' ? <div className="PopupInfoArea">
                ...
                {Object.keys(fieldMap).map((key: any) => (
                    <div className="PopupInfoBlock">
                        <div className="PopupInfoTitle">{translate[fieldMap[key].label]?.[props.getLang] ?? fieldMap[key].label}</div>
                        <div className="PopupInfoValue">{fieldMap[key]?.format ? fieldMap[key].format(props.actiom.data[key], props.getLang) : props.actiom.data[key]}</div>
                    </div>
                ))}
            </div>
                :
                <div className="FilterArea">
                    <div className="FilterBlock">
                        ...
                        <div className="FilterInputBlock">
                            ...
                            <Calendar
                                ...
                                weekDays={props.getLang === "EN" ? ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"] : props.getLang === "UZ" ? ["Du", "Se", "Ch", "Pa", "Ju", "Sh", "Ya"] : ["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"]}
                                monthNames={props.getLang === "EN" ? ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"] : props.getLang === "UZ" ? ["Yan", "Fev", "Mar", "Apr", "May", "Iyn", "Iyl", "Avg", "Sen", "Okt", "Noy", "Dek"] : ["Янв", "Фев", "Мар", "Апр", "Май", "Июн", "Июл", "Авг", "Сен", "Окт", "Ноя", "Дек"]}
                                className="FilterDate"
                                placeholder={props.getLang === "EN" ? "DD.MM.YYYY" : "KK.OO.YYYY"}
                                format="%D.%M.%Y"
                            />
                        </div>
                        <div className="FilterInputBlock">
                    ...
                    <div className="FilterBlock">
                        <div className="FilterTitle">{translate["Fazoviy aniqligi"][props.getLang]} (m/pixel):</div>
